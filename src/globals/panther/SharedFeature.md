...
Every node also carries an `extras` common property — arbitrary
application-specific key-value data that has no dedicated property, stored as
a JSON string. When a request provides `extras` as an object, the parser
normalizes it to a JSON string; string values pass through unchanged. It is
`null` when the request does not provide it.
